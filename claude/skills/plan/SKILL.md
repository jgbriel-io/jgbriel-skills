...
Usage: `/plan <task, brief or investigation>`, or the agent's native skill invocation.

Plan the work that `implement` will execute. Deliver the plan **in the conversation**. Do not create documents, plan directories, feature records or code.
...
- Clarify material open product decisions one at a time, or recommend `research` for investigation. Do not hide unresolved research inside implementation steps.
...
Present the objective, acceptance criteria, steps, risks, checks and open items in the conversation. Identify execution blockers before suggesting `implement`.
