...
Usage: `/diagnose <symptom> [--fix]`, or the agent's native skill invocation.
...
## Local addition: HITL escalation

Last resort when a human must click through a step: drive them with `scripts/hitl-loop.template.sh` so the loop stays structured. Captured output feeds back into the diagnosis.
