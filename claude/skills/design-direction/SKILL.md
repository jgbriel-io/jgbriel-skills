...
### 5.A–5.C Canonical Animation Skeletons

When implementing pin/scrub or scroll-reveal work (Sticky-Stack, Horizontal-Pan, Scroll-Reveal Stagger), read [references/animation-skeletons.md](references/animation-skeletons.md) and copy the canonical skeletons. Do not improvise ScrollTrigger wiring from memory — the common failures (trigger firing mid-scroll instead of pinning at top, missing cleanup) are already solved there.
...
Full level-by-level definitions of `DESIGN_VARIANCE`, `MOTION_INTENSITY`, and `VISUAL_DENSITY`: [references/dials-reference.md](references/dials-reference.md). Read when a dial value is contested or ambiguous.
...
Mandatory read before the Final Pre-Flight Check: [references/ai-tells.md](references/ai-tells.md) — visual/CSS, typography, layout, content ("Jane Doe" effect), external-resource and production-test tells, plus the EM-DASH BAN (the single most-violated tell: em-dashes are completely banned in visible page copy).

## 10. REFERENCE VOCABULARY (Pattern Names)

Named hero paradigms, navigation patterns, grids, card systems, scroll animations, galleries, micro-interactions, and animation-library choice: [references/pattern-vocabulary.md](references/pattern-vocabulary.md). Read when you need a named pattern or the user references one.

## 11. REDESIGN PROTOCOL

For redesign briefs: [references/redesign-protocol.md](references/redesign-protocol.md) — mode detection (preserve vs overhaul), audit-before-touching, preservation rules, modernisation levers, decision tree. For incremental upgrades of an existing production codebase, the `redesign-existing-projects` skill is the dedicated tool.

## 12. THE BLOCK LIBRARY (Contract)

Block-library file location, required frontmatter, and discipline: [references/block-library.md](references/block-library.md). Read when saving or reusing section blocks.
...
Install commands per design system and canonical documentation links (Material, Fluent, Carbon, Polaris, Atlaskit, Primer, GOV.UK, USWDS, Bootstrap, Tailwind, Radix, shadcn/ui, Liquid Glass approximation): [references/design-system-sources.md](references/design-system-sources.md). Read before installing any package from Section 2.
