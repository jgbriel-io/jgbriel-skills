---
name: fable-method
description: An evidence-first problem-solving loop for work outside code — money and pricing decisions, marketing and content, research and reporting, data analysis, business and ops, legal and compliance, design and UX. Each domain brings a binding minimum evidence set, an authority order and a fraud table, so a figure or claim never ships on memory. Use when the task is in one of those domains, when the user says "/fable-method", "use the fable method" or "approach this like Fable", or to audit finished work against the loop. Software work belongs to discuss, research, plan, implement and diagnose, which go deeper; reach for this one when no task-specific skill fits. Subcommands - plan (stop after the plan), audit (grade finished work), report (rewrite an answer outcome-first).
...
**Domain adapters.** Reading the matching adapter is the normal path, not an extra: this loop earns its keep in the domains below, where a wrong figure or an unsourced claim is the failure mode. Coding still runs on the generic defaults, but it is the weakest reason to be here — `discuss`, `research`, `plan`, `implement` and `diagnose` cover software work far deeper. Prefer them for anything squarely inside that path, and keep this loop for the domains below, for work spanning several of them, or when no task-specific skill fits. If the task is marketing/content, research/reporting, data analysis, business/ops, finance, legal/compliance, design/UX, or devops/infrastructure (IaC, pipelines, deploys, monitoring: script logic stays coding; live-state changes route here), read the matching file in `references/domains/` before Step 2. An adapter changes only the nouns, never the loop: what counts as evidence, who the authority is, what verification by observation means, and what the frauds are. Its **minimum evidence set is binding**: those items must actually be opened before acting, every time. Research is never optional; the adapter defines how much is enough. Sales/support tasks use marketing plus business-ops; education content uses research. Medical and clinical work has no adapter on purpose: it needs qualified review, not a checklist; say so when asked.
